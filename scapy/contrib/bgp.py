...
class ASPathSegment(Packet):
    name = "AS_PATH Segment"
    fields_desc = [
        ByteEnumField("ps_type", 2, {1:"AS_SET", 2:"AS_SEQUENCE"}),
        FieldLenField("ps_count", None, fmt="!B", count_of="ps_value"),
        FieldListField("ps_value", [], ShortField("", 0),
            count_from=lambda pkt: pkt.ps_count),
        ]

    def post_build(self, p, pay):
        if self.ps_value and self.ps_count is None:
            l = len(self.ps_value)
            p = p[:1] + struct.pack("!B",l) + p[2:]
        return p+pay
...
class Aggregator(Packet):
    name = "AGGREGATOR Attribute"
    fields_desc = [
        ShortField("last_as_number", 0),
...
class BGPPathAttribute(Packet):
    ...
    fields_desc = [
    FlagsField("flags", 0x40, 8, ["NA0","NA1","NA2","NA3",
        "Extended-Length","Partial","Transitive","Optional"]),
    ByteEnumField("type", None, {1:"ORIGIN", 2:"AS_PATH", 3:"NEXT_HOP",
        4:"MULTI_EXIT_DISC", 5:"LOCAL_PREF", 6:"ATOMIC_AGGREGATE",
        7:"AGGREGATOR"}),
    ConditionalField(
        ByteField("len", None),
        cond=lambda pkt: (pkt.flags>>4)&1 == 0),
    ConditionalField(
        ShortField("extlen", None),
        cond=lambda pkt: (pkt.flags>>4)&1 == 1),

    # ORIGIN
    ConditionalField(
        ByteEnumField("origin", 0, {0:"IGP", 1:"EGP", 2:"INCOMPLETE"}),
        cond=lambda pkt:pkt.type==1),
    # AS_PATH
    ConditionalField(
        PacketField("as_path", ASPathSegment(), ASPathSegment),
        cond=lambda pkt:pkt.type==2),
    # NEXT_HOP
    ConditionalField(
        IPField("next_hop", "0.0.0.0"),
        cond=lambda pkt:pkt.type==3),
    # MULTI_EXIT_DISC
    ConditionalField(
        IntField("multi_exit_disc", 0),
        cond=lambda pkt:pkt.type==4),
    # LOCAL_PREF
    ConditionalField(
        IntField("local_pref", 0),
        cond=lambda pkt:pkt.type==5),
    # ATOMIC_AGGREGATE is of length 0 (no value)
    # AGGREGATOR
    ConditionalField(
        PacketField("aggregator", Aggregator(), Aggregator),
        cond=lambda pkt:pkt.type==7),
    ]

    def post_build(self, p, pay):
        if self.len is None and self.extlen is None:
            flags = ord(p[0:1])
            l = len(p) + len(pay)
            if l - 4 > 0xFFFF:
                raise Error("Packet is too large")
            elif l - 4 > 0xFF:
                l -= 4
                flags |= 1<<4
                p = chr(flags) + p[1:2] + struct.pack("!H", l) + p[4:]
            else:
                l -= 3
                flags &= ~(1<<4)
                p = chr(flags) + p[1:2] + struct.pack("!B", l)  +p[3:]
        return p+pay
...
class BGPUpdate(Packet):
    ...
    name = "BGP Update"
    fields_desc = [
        FieldLenField("withdrawn_len", 0,
            length_of=lambda p: p.withdrawn),
        FieldListField("withdrawn", [], BGPIPField("","0.0.0.0/0"),
            length_from=lambda p:p.withdrawn_len),
        FieldLenField("tp_len", 0,
            length_of=lambda p: p.total_path),
