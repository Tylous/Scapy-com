...
class RandMAC(RandString):
    """Generates a random MAC address each time an instance is
    evaluated.  Evaluate it manually by calling str() on it, or put it
    directly in a packet to generate a new random MAC address each
    time the packet is evaluated.

    Call str() on it to get a single random MAC address. Call str() on
    it again to get a different random MAC address.

    If the template field is set to a string containing a byte value
    in hex, it will be used as the first byte of all MAC addresses
    generated by the instance returned. The template field also
    accepts ranges in the form 'A-B' where 'A' and 'B' are hex
    values. This will fill in the first byte of the MAC address with a
...
class RandIP6(RandString):
    """Generates a random IPv6 address each time an instance is
    evaluated.  Evaluate it manually by calling str() on it, or put it
    directly in a packet to generate a new random IPv6 address each
    time the packet is evaluated.

    The ip6template field has a particular format. It can contain up
    to eight colon-separated fields, each of which can have one of
    four formats. Valid formats include:

    **  - Fill the rest of the address with random values.
    *   - Fill this two-byte section of the address with a random value.
    A-B - Fill this two-byte section of the address with a random value in the range [A,B].
    A   - Put the hex number A in this two-byte section of the address.

    For example, RandIP6('foof:3-ffff:*:1:**') will generate addresses
    foof:X:Y:1:Y:Y:Y:Y where X is a random number in the range
    [0x3,0xFFFF] and every instance of Y is a random number in the
    range [0,0xFFFF]."""
